...
export default function DesignError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => { console.error('[DesignError]', error); }, [error]);
  return (
    <main className="min-h-screen bg-background">
      <SiteHeader />
      <div className="flex flex-col items-center justify-center py-32 text-center gap-4">
        <AlertTriangle size={32} className="text-destructive" />
        <p className="text-xl font-semibold">خطا در بارگذاری طراحی</p>
        <p className="text-sm text-muted-foreground">Failed to load design. Please try again.</p>
