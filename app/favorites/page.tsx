      ...
      <section className="border-b border-border/60 bg-[#f2efe8]">
        <div className="mx-auto max-w-[1440px] px-5 py-12 sm:px-8 lg:px-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            {savedCount} {isRTL ? '' : (favoritedDesigns.length === 1 ? 'design' : 'designs')} {isRTL ? dict.favorites.savedCount.replace('{count}', '') : "you've saved"}
          </p>
        </div>
      </section>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="mt-2 max-w-sm text-sm text-muted-foreground">
              {dict.favorites.noFavoritesDesc}
            </p>
            <Link
              href={`${base}/discover`}
            ...
            >
              {dict.favorites.discoverDesigns} {isRTL ? <ArrowRight size={16} className="transition-transform group-hover:-translate-x-1 rotate-180" /> : <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />}
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-x-4 gap-y-9 md:grid-cols-3 lg:grid-cols-4 lg:gap-x-6">
